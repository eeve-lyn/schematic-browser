...
const planetFilter = (schem) => {
    let serpulo = schem.requirements().toSeq().anyMatch(i => Items.serpuloItems.contains(i.item) && !Items.erekirItems.contains(i.item));
    let erekir = schem.requirements().toSeq().anyMatch(i => Items.erekirItems.contains(i.item) && !Items.serpuloItems.contains(i.item));
